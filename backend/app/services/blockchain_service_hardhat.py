...
def mint_ticket(token_id: int, event_id: int, seat_id: Optional[str], ticket_hash: str) -> str:
    ...
    try:
        ...
        # Idempotency: if already minted, accept
        try:
            got = get_ticket(token_id)
            if got and got.get("ticketHash") == ticket_hash:
...
def get_ticket(token_id: int) -> Optional[Dict[str, Any]]:
    _, c = _init()
    try:
        # Expect tuple: (eventId, seatId, ticketHash, used)
        event_id, seat_id, ticket_hash, used = c.functions.getTicket(int(token_id)).call()
        # Some contracts may return default zeros when absent; normalize to None if empty hash
        if isinstance(ticket_hash, bytes):
            # Convert to 0x hex
            ticket_hash = Web3.to_hex(ticket_hash)
        if ticket_hash in (None, "", "0x" + "00" * 32):
            return None
        return {"eventId": int(event_id), "seatId": seat_id, "ticketHash": ticket_hash, "used": bool(used)}
    except Exception:
        # If token not present, treat as None
        return None
